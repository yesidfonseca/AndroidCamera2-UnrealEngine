...
public:
	...
	/**
	 * Positions of the global mesh. Only populated if the anchor is has the label GLOABL_MESH
	 * and the option UseSharedLibraryForAnchorLoading is set to true.
	 */
	UPROPERTY(VisibleInstanceOnly, Transient, BlueprintReadOnly, Category = "MR Utility Kit")
	TArray<FVector> SceneMeshPositions;

	/**
	 * Indices of the global mesh. Only populated if the anchor is has the label GLOABL_MESH
	 * and the option UseSharedLibraryForAnchorLoading is set to true.
	 */
	UPROPERTY(VisibleInstanceOnly, Transient, BlueprintReadOnly, Category = "MR Utility Kit")
	TArray<int> SceneMeshIndices;
...
public:
	...
	void Setup(const FOculusXRUUID& Uuid, FOculusXRUInt64 Space, const FTransform& Transform, const TArray<FString>& Labels,
