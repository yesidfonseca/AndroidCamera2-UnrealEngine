	...
	ButtonWidget->AddSlot()
		...
		.Padding(FMargin(8.f, 0.f, 8.f, 0.f))
			[SNew(SButton)
					...
					.ToolTipText(LOCTEXT("FeedbackToolTip", "Give feedback or report bugs"))
...
void SOculusXRProjectSetupToolWidget::BuildRowItemLayout(const TSharedPtr<SVerticalBox>& SectionContentContainer, ERulesSection Section, const SetupRulePtr& Rule, const uint32 PlatformFilterIndex)
{
	...
	const bool bShowMoreInfoButton = (!Rule->ShowApply());
		...
		ButtonWidget->AddSlot()
			.Padding(FMargin(6, 3, 3, 3))
				[SNew(SButton)
						.HAlign(HAlign_Center)
						.VAlign(VAlign_Center)
						.Text(LOCTEXT("MoreInfo", "More Info"))
						.Visibility_Lambda([bShouldHideApplyButton]() -> EVisibility { return bShouldHideApplyButton ? EVisibility::Hidden : EVisibility::Visible; })
						.OnClicked(this, &SOculusXRProjectSetupToolWidget::OnMoreInfoClicked, Rule)
						.IsEnabled_Static(&SOculusXRProjectSetupToolWidget::OnApplyRuleEnabled, Section)];
	}
	...
	FString SupportURL(TEXT("https://forums.oculusvr.com/developer"));

	if (!Rule->GetInfoUrl().IsEmpty())
	{
		SupportURL = Rule->GetInfoUrl();
	}
...
FReply SOculusXRProjectSetupToolWidget::OnReportIssueClicked()
{
	OculusXRTelemetry::TScopedMarker<OculusXRTelemetry::Events::FProjectSetupToolOption> ReportIssueEvent;
	const auto& _ = ReportIssueEvent.AddAnnotation("ToolName", "ProjectSetupTool");

	const FString MQDHDeepLink("odh://feedback-hub?showSubmitFeedback&platformID=2283488761985181&categoryID=482296384788650");

	FString error = "";
	FPlatformProcess::LaunchURL(*MQDHDeepLink, nullptr, &error);
	if (!error.IsEmpty())
	{
		ReportIssueEvent.SetResult(OculusXRTelemetry::EAction::Fail);
		FNotificationInfo Info(LOCTEXT("InstallMQDH", "Install Meta Quest Developer Hub"));
		Info.ExpireDuration = 30.0f;
		Info.Hyperlink = FSimpleDelegate::CreateLambda([]() {
			FPlatformProcess::LaunchURL(TEXT("https://developers.meta.com/horizon/documentation/unity/ts-odh-getting-started/"), nullptr, nullptr);
		});
		Info.SubText = LOCTEXT("Description", "Meta Quest Developer Hub is not installed on this machine.");
		Info.HyperlinkText = LOCTEXT("DownloadUrl", "Get Meta Quest Developer Hub");
 ...
 */
FReply SOculusXRProjectSetupToolWidget::OnMoreInfoClicked(SetupRulePtr Rule)
{
	if (Rule == nullptr)
	{
		return FReply::Handled();
	...
	if (Rule->GetInfoUrl().IsEmpty())
	{
		...
		FPlatformProcess::LaunchURL(*SupportURL, nullptr, nullptr);
	}
	else
	{
		FPlatformProcess::LaunchURL(*Rule->GetInfoUrl(), nullptr, nullptr);
	}
...
FReply SOculusXRProjectSetupToolWidget::OnApplyAllRulesClicked(ERulesSection Section)
{
	...
	for (auto& Rule : RuleProcessorSubsystem->GetRules())
	{
		...
		// Only apply rules that show Apply button
		bShouldApplyRule = bShouldApplyRule && Rule->ShowApply();
		if (!bShouldApplyRule)
		{
			continue;
		}
