...
namespace OculusXRAnchors
{
	...
	{
		OutResult = FOculusXRAnchorManager::DestroyAnchor(AnchorHandle);
	...
	bool FOculusXRAnchors::SetAnchorComponentStatus(UOculusXRAnchorComponent* Anchor, EOculusXRSpaceComponentType SpaceComponentType, bool Enable, float Timeout, const FOculusXRAnchorSetComponentStatusDelegate& ResultCallback, EOculusXRAnchorResult::Type& OutResult)
	{
		...
		uint64 RequestId = 0;
		OutResult = FOculusXRAnchorManager::SetAnchorComponentStatus(Anchor->GetHandle(), SpaceComponentType, Enable, Timeout, RequestId);
		bool bAsyncStartSuccess = UOculusXRAnchorBPFunctionLibrary::IsAnchorResultSuccess(OutResult);
	...
	{
		uint64 RequestId = 0;
		OutResult = FOculusXRAnchorManager::SetAnchorComponentStatus(Space, SpaceComponentType, Enable, Timeout, RequestId);
		bool bAsyncStartSuccess = UOculusXRAnchorBPFunctionLibrary::IsAnchorResultSuccess(OutResult);
	...
	{
		OutResult = FOculusXRAnchorManager::GetAnchorComponentStatus(AnchorHandle, SpaceComponentType, OutEnabled, OutChangePending);
		return UOculusXRAnchorBPFunctionLibrary::IsAnchorResultSuccess(OutResult);
	}
	...
	{
		uint64 RequestId = 0;
		OutResult = FOculusXRAnchorManager::QueryAnchors(QueryInfo, RequestId);
		bool bAsyncStartSuccess = UOculusXRAnchorBPFunctionLibrary::IsAnchorResultSuccess(OutResult);
	...
	{
		uint64 RequestId = 0;
		OutResult = FOculusXRAnchorManager::ShareAnchors(AnchorHandles, OculusUserIDs, RequestId);
		bool bAsyncStartSuccess = UOculusXRAnchorBPFunctionLibrary::IsAnchorResultSuccess(OutResult);
	...
	bool FOculusXRAnchors::ShareAnchors(const TArray<UOculusXRAnchorComponent*>& Anchors, const TArray<uint64>& OculusUserIDs, const FOculusXRAnchorShareDelegate& ResultCallback, EOculusXRAnchorResult::Type& OutResult)
	{
		...
		uint64 RequestId = 0;
		OutResult = FOculusXRAnchorManager::ShareAnchors(Handles, OculusUserIDs, RequestId);
		bool bAsyncStartSuccess = UOculusXRAnchorBPFunctionLibrary::IsAnchorResultSuccess(OutResult);
	...
	bool FOculusXRAnchors::SaveAnchors(const TArray<UOculusXRAnchorComponent*>& Anchors, const FOculusXRSaveAnchorsDelegate& ResultCallback, EOculusXRAnchorResult::Type& OutResult)
	{
		...
		uint64 RequestId = 0;
		OutResult = FOculusXRAnchorManager::SaveAnchors(Handles, RequestId);
		bool bAsyncStartSuccess = UOculusXRAnchorBPFunctionLibrary::IsAnchorResultSuccess(OutResult);
	...
	bool FOculusXRAnchors::EraseAnchors(const TArray<UOculusXRAnchorComponent*>& Anchors, const FOculusXREraseAnchorsDelegate& ResultCallback, EOculusXRAnchorResult::Type& OutResult)
	{
		...
		uint64 RequestId = 0;
		OutResult = FOculusXRAnchorManager::EraseAnchors(TArray<FOculusXRUInt64>(), UUIDs, RequestId);
		bool bAsyncStartSuccess = UOculusXRAnchorBPFunctionLibrary::IsAnchorResultSuccess(OutResult);
	...
	{
		uint64 RequestId = 0;
		OutResult = FOculusXRAnchorManager::EraseAnchors(AnchorHandles, AnchorUUIDs, RequestId);
		bool bAsyncStartSuccess = UOculusXRAnchorBPFunctionLibrary::IsAnchorResultSuccess(OutResult);
	...
	{
		uint64 RequestId = 0;
		OutResult = FOculusXRAnchorManager::DiscoverAnchors(DiscoveryInfo, RequestId);
		bool bAsyncStartSuccess = UOculusXRAnchorBPFunctionLibrary::IsAnchorResultSuccess(OutResult);
	...
	{
		OutResult = FOculusXRAnchorManager::GetAnchorContainerUUIDs(Space, OutUUIDs);
		return UOculusXRAnchorBPFunctionLibrary::IsAnchorResultSuccess(OutResult);
	}
	...
	TSharedPtr<FGetAnchorsSharedWithGroup> FOculusXRAnchors::GetSharedAnchorsAsync(const FOculusXRUUID& Group, const TArray<FOculusXRUUID>& WantedAnchors, const FGetAnchorsSharedWithGroup::FCompleteDelegate& OnComplete)
	{
		auto request = OculusXR::FAsyncRequestSystem::CreateRequest<FGetAnchorsSharedWithGroup>(Group, WantedAnchors);
		request->BindOnComplete(OnComplete);
		request->Execute();
		return request;
	}
	...
	void FOculusXRAnchors::HandleSpatialAnchorCreateComplete(FOculusXRUInt64 RequestId, EOculusXRAnchorResult::Type Result, FOculusXRUInt64 Space, FOculusXRUUID UUID)
	{
		OculusXRTelemetry::Events::FAnchorsCreateResponse(static_cast<int>(GetTypeHash(RequestId)))
			.SetResult(UOculusXRAnchorBPFunctionLibrary::IsAnchorResultSuccess(Result) ? OculusXRTelemetry::EAction::Success : OculusXRTelemetry::EAction::Fail);
		...
		if (!UOculusXRAnchorBPFunctionLibrary::IsAnchorResultSuccess(Result))
		{
			...
			AnchorDataPtr->Binding.ExecuteIfBound(Result, nullptr);
		...
		uint64 tempOut;
		FOculusXRAnchorManager::SetAnchorComponentStatus(Space, EOculusXRSpaceComponentType::Locatable, true, 0.0f, tempOut);
		FOculusXRAnchorManager::SetAnchorComponentStatus(Space, EOculusXRSpaceComponentType::Sharable, true, 0.0f, tempOut);
		FOculusXRAnchorManager::SetAnchorComponentStatus(Space, EOculusXRSpaceComponentType::Storable, true, 0.0f, tempOut);

		AnchorDataPtr->Binding.ExecuteIfBound(Result, SpatialAnchorComponent);
		CreateSpatialAnchorBindings.Remove(RequestId.GetValue());
	}

	void FOculusXRAnchors::HandleAnchorEraseComplete(FOculusXRUInt64 RequestId, EOculusXRAnchorResult::Type Result, FOculusXRUUID UUID, EOculusXRSpaceStorageLocation Location)
	{
		...
		if (!UOculusXRAnchorBPFunctionLibrary::IsAnchorResultSuccess(Result))
		{
			...
			EraseDataPtr->Binding.ExecuteIfBound(Result, UUID);
	...
	void FOculusXRAnchors::HandleSetComponentStatusComplete(FOculusXRUInt64 RequestId, EOculusXRAnchorResult::Type Result, FOculusXRUInt64 Space, FOculusXRUUID UUID, EOculusXRSpaceComponentType ComponentType, bool Enabled)
	{
		...
		if (SetStatusBinding != nullptr)
		{
			SetStatusBinding->Binding.ExecuteIfBound(Result, SetStatusBinding->AnchorHandle, ComponentType, Enabled);
	...
	void FOculusXRAnchors::HandleAnchorSaveComplete(FOculusXRUInt64 RequestId, FOculusXRUInt64 Space, bool Success, EOculusXRAnchorResult::Type Result, FOculusXRUUID UUID)
	{
		...
		if (!UOculusXRAnchorBPFunctionLibrary::IsAnchorResultSuccess(Result))
		{
			...
			SaveAnchorData->Binding.ExecuteIfBound(Result, SaveAnchorData->Anchor.Get());
	...
	void FOculusXRAnchors::HandleAnchorSaveListComplete(FOculusXRUInt64 RequestId, EOculusXRAnchorResult::Type Result)
	{
		...
		if (!UOculusXRAnchorBPFunctionLibrary::IsAnchorResultSuccess(Result))
		{
			...
			SaveListData->Binding.ExecuteIfBound(Result, SavedAnchors);
	...
	void FOculusXRAnchors::HandleAnchorQueryComplete(FOculusXRUInt64 RequestId, EOculusXRAnchorResult::Type Result)
	{
	...
	void FOculusXRAnchors::QuerySpacesComplete(AnchorQueryBinding* Binding, FOculusXRUInt64 RequestId, EOculusXRAnchorResult::Type Result)
	{
		Binding->Binding.ExecuteIfBound(Result, Binding->Results);
		AnchorQueryBindings.Remove(RequestId.GetValue());
	}

	void FOculusXRAnchors::GetSharedAnchorsComplete(GetSharedAnchorsBinding* Binding, FOculusXRUInt64 RequestId, EOculusXRAnchorResult::Type Result)
	{
		Binding->ResultBinding.ExecuteIfBound(Result, Binding->Results);
		GetSharedAnchorsBindings.Remove(RequestId.GetValue());
	}

	void FOculusXRAnchors::HandleAnchorSharingComplete(FOculusXRUInt64 RequestId, EOculusXRAnchorResult::Type Result)
	{
		...
		ShareAnchorsData->Binding.ExecuteIfBound(Result, SharedAnchors, ShareAnchorsData->OculusUserIds);
		ShareAnchorsBindings.Remove(RequestId.GetValue());
	}

	void FOculusXRAnchors::HandleAnchorsSaveComplete(FOculusXRUInt64 RequestId, EOculusXRAnchorResult::Type Result)
	{
		...
		{
			UE_LOG(LogOculusXRAnchors, Warning, TEXT("Failed to save Spatial Anchors. Request: %llu  --  Result: %d"), RequestId.GetValue(), Result);
			SaveData.Binding.ExecuteIfBound(Result, SavedAnchors);
			return;
		}
	...
	void FOculusXRAnchors::HandleAnchorsEraseComplete(FOculusXRUInt64 RequestId, EOculusXRAnchorResult::Type Result)
	{
	...
	void FOculusXRAnchors::HandleAnchorsEraseByComponentsComplete(FOculusXRUInt64 RequestId, EOculusXRAnchorResult::Type Result)
	{
		...
		if (!UOculusXRAnchorBPFunctionLibrary::IsAnchorResultSuccess(Result))
		{
			...
			EraseData.Binding.ExecuteIfBound(Result, ErasedAnchors, TArray<FOculusXRUInt64>(), TArray<FOculusXRUUID>());
			return;
		}
	...
	void FOculusXRAnchors::HandleAnchorsEraseByHandleAndUUIDComplete(FOculusXRUInt64 RequestId, EOculusXRAnchorResult::Type Result)
	{
		EraseAnchorsBinding EraseData;
		if (!EraseAnchorsBindings.RemoveAndCopyValue(RequestId.GetValue(), EraseData))
		{
		...
		EraseData.Binding.ExecuteIfBound(Result, TArray<UOculusXRAnchorComponent*>(), EraseData.ErasedAnchorsHandles, EraseData.ErasedAnchorsUUIDs);
	}
	...
	void FOculusXRAnchors::HandleAnchorsDiscoverResults(FOculusXRUInt64 RequestId, const TArray<FOculusXRAnchorsDiscoverResult>& Results)
	{
		...
		for (auto& Result : Results)
		{
			...
			if (supportedTypes.Contains(EOculusXRSpaceComponentType::Locatable))
			{
				FOculusXRAnchorManager::SetAnchorComponentStatus(Result.Space, EOculusXRSpaceComponentType::Locatable, true, 0.0f, tempOut);
			}

			if (supportedTypes.Contains(EOculusXRSpaceComponentType::Sharable))
			{
				FOculusXRAnchorManager::SetAnchorComponentStatus(Result.Space, EOculusXRSpaceComponentType::Sharable, true, 0.0f, tempOut);
			}

			if (supportedTypes.Contains(EOculusXRSpaceComponentType::Storable))
			{
				FOculusXRAnchorManager::SetAnchorComponentStatus(Result.Space, EOculusXRSpaceComponentType::Storable, true, 0.0f, tempOut);
			}
		}
	...
	void FOculusXRAnchors::HandleAnchorsDiscoverComplete(FOculusXRUInt64 RequestId, EOculusXRAnchorResult::Type Result)
	{
		AnchorDiscoveryBinding DiscoveryData;
		...
		DiscoveryData.CompleteBinding.ExecuteIfBound(Result);
	}
